# grid_vit_cifar10_gpu_full.py
# Full ViT-Small for CIFAR-10 with grid search on GPU

import math, time, itertools, random, argparse
...
def train_one_epoch(model, loader, optimizer, scaler, device, scheduler=None):
    ...
    for images, targets in loader:
        ...
        scaler.update()
        # if you want scheduler stepped per batch keep this:
...
def run_training(cfg:RunConfig, epochs:int, warmup_epochs:int, min_lr:float, device, num_workers:int=4):
    train_loader, test_loader = build_dataloaders(cfg.batch_size, num_workers)
    model = ViTSmallCIFAR(drop_path_rate=cfg.drop_path_rate).to(device)
    ...
    training_log, best_acc = [], 0.0
    for epoch in range(epochs):
        train_loss, train_acc = train_one_epoch(model, train_loader, optimizer, scaler, device, scheduler=scheduler)
        test_acc = evaluate(model, test_loader, device)
        best_acc = max(best_acc, test_acc)
        print(f"epoch {epoch+1}/{epochs} | loss {train_loss:.4f} | train_acc {train_acc*100:.2f}% | test_acc {test_acc*100:.2f}%")
        training_log.append([cfg.lr, cfg.weight_decay, cfg.batch_size, cfg.drop_path_rate, train_loss, train_acc, test_acc])
    return {"config":cfg, "best_acc":best_acc, "training_log":training_log}
...
def run_one_hypertune(seed):
    ...
    parser.add_argument("--dprs",type=float,nargs="+",default=[0.10,0.20,0.40])
    ...
    results,best_training_log,best_acc = [],None,float('-inf')

    for i,(lr,wd,bs,dpr) in enumerate(search_space,1):
        print(f"\n=== run {i}/{len(search_space)} | lr={lr} wd={wd} bs={bs} drop_path={dpr} ===")
        cfg = RunConfig(lr=lr,weight_decay=wd,batch_size=bs,drop_path_rate=dpr)
        res = run_training(cfg,epochs=args.epochs,warmup_epochs=args.warmup_epochs,min_lr=args.min_lr,device=device,num_workers=args.num_workers)
        if res['best_acc'] > best_acc:
            best_training_log = res['training_log']
        results.append({"config":res['config'],"best_acc":res['best_acc']})

    training_logs = pd.DataFrame(best_training_log,columns=['lr','wd','bs','dpr','train_loss','train_acc','test_acc'])
    ...
    for rank,r in enumerate(results,1):
        cfg = r["config"]
        print(f"{rank:2d}) acc={r['best_acc']*100:.2f}% | lr={cfg.lr} wd={cfg.weight_decay} bs={cfg.batch_size} dpr={cfg.drop_path_rate}")
